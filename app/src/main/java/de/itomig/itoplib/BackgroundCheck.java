 ...
 */
	protected class RequestTicketsFromServerTask extends AsyncTask<String, Void, ArrayList<ItopTicket>> {
		...
		protected void onPostExecute (ArrayList<ItopTicket> resTickets) {
			...
			bgtickets = ItopUtils.removeRepeatingTickets(bgtickets);
			if (debug) { 
			...
			if (bgtickets.size()>0 && (!bgtickets.get(0).getType().equals(ERROR))) createNewIncidentNotification(); 
	...
	private void checkStop() {
		if (!reqRunningFlag)  {	
			...
			if (debug) Log.i(TAG,"BG: checkStop cannot stop the service! still active http requests.");
		}
	}
