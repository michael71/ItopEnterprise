...
public class Cache {
    ...
    public static void cachePersons(Context context) {
        ...
        Type listType = new TypeToken<ConcurrentHashMap<Integer, Person>>() {
        }.getType();
    ...
    public static boolean getCachePersons(Context context) {
        ...
        if ((System.currentTimeMillis() - ctPersons) < MAX_CACHE_LIFETIME) {
            ...
            if (debug)
                Log.d(TAG, "CACHE - reading persons.  " + DateFormat.format("dd.MM - HH:mm:ss", personLookupTime));
            return true;
        } else {
            if (debug)
                Log.d(TAG, "CACHETIME-PERSONS too old: " + DateFormat.format("dd.MM - HH:mm:ss", personLookupTime));
            return false;
        }
    ...
    public static boolean getCacheOrgs(Context context) {
        ...
        if ((System.currentTimeMillis() - ctOrgs) < MAX_CACHE_LIFETIME) {
            ...
            if (debug)
                Log.d(TAG, "reading orgs cache. datetime=" + DateFormat.format("dd.MM - HH:mm:ss", organizationLookupTime));
            return true;
        } else {
            if (debug)
                Log.d(TAG, "CACHETIME-ORGS too old: " + DateFormat.format("dd.MM - HH:mm:ss", organizationLookupTime));
            return false;
        }

    }

    public static synchronized void clearPersonsCache(Context context) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
    ...
    public static synchronized void clearOrgsCache(Context context) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
    ...
    public static synchronized void write(Context context, long timemillis, String name, Object obj, Type listType) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        Gson gson = new Gson();
        ...
        if (debug)
            Log.d(TAG, "CACHETIME-" + name + ": " + DateFormat.format("dd.MM - hh:mm:ss", timemillis));
        prefs.edit().putLong("CACHETIME-" + name, timemillis).commit();

    }

    public static synchronized Object read(Context context, String name, Object obj, Type listType) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        ...
        if ((System.currentTimeMillis() - timemillis) < MAX_CACHE_LIFETIME) {
            ...
            obj = gson.fromJson(
                    json, listType);
            if (debug)
                Log.d(TAG, "CACHETIME- was: " + DateFormat.format("dd.MM - hh:mm:ss", timemillis));
        } else {
            obj = null;
            if (debug)
                Log.d(TAG, "CACHETIME- was too old: " + DateFormat.format("dd.MM - hh:mm:ss", timemillis));
        }
